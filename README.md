...
sys.path.append('/opt/homebrew/lib/python2.7/site-packages/te_fb-3.0.0-py2.7.egg-info') # same as above
...
There is and additional folder ``adobjects`` under te_fb. Under this you will see a file for every ad object
...
>>> type(my_accounts[0])
<class 'te_fb.adobjects.AdAccount'>
```
...
``te_fb.adobjects``) and we want to get the full list right away instead of
...
descriptions are listed in ``te_fb.exceptions``.
...
of ``FacebookAdsApiBatch`` (available in te_fb.api). To easily get one
...
See ``te_fb.exceptions`` for a list of exceptions which may be thrown by
the SDK.
