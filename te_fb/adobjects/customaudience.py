...
from te_fb.adobjects.helpers.customaudiencemixin import CustomAudienceMixin
...
class CustomAudience(
    ...
    def get_sessions(self, fields=None, params=None, batch=None, success=None, failure=None, pending=False):
        ...
        if batch is None and (success is not None or failure is not None):
          api_utils.warning('`success` and `failure` callback only work for batch call.')
        from te_fb.adobjects.customaudiencesession import CustomAudienceSession
        param_types = {
    ...
    def get_shared_account_info(self, fields=None, params=None, batch=None, success=None, failure=None, pending=False):
        ...
        if batch is None and (success is not None or failure is not None):
          api_utils.warning('`success` and `failure` callback only work for batch call.')
        from te_fb.adobjects.customaudiencesharedaccountinfo import CustomAudiencesharedAccountInfo
        param_types = {
        }
        enums = {
        }
